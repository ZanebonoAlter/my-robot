...
func (s *AutoRefreshScheduler) UpdateInterval(interval int) error {
	...
	if err := repository.Repo.DB().Where("name = ?", "auto_refresh").First(&task).Error; err == nil {
...
func (s *AutoRefreshScheduler) GetStatus() adminhandler.SchedulerStatusResponse {
...
func (s *AutoRefreshScheduler) updateSchedulerStatus(status, lastError string, startTime *time.Time, summary *AutoRefreshRunSummary) {
	var task models.SchedulerTask
	err := repository.Repo.DB().Where("name = ?", "auto_refresh").First(&task).Error
	now := time.Now()
