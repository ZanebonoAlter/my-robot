...
func (s *ContentCompletionScheduler) UpdateInterval(interval int) error {
	...
	if err := repository.Repo.DB().Where("name = ?", s.taskName).First(&task).Error; err == nil {
		nextRun := time.Now().Add(s.checkInterval)
		repository.Repo.DB().Model(&task).Updates(map[string]interface{}{
...
func (s *ContentCompletionScheduler) initSchedulerTask() {
	var task models.SchedulerTask
	err := repository.Repo.DB().Where("name = ?", s.taskName).First(&task).Error
...
func (s *ContentCompletionScheduler) reconcileSchedulerTask() error {
	...
	primaryErr := repository.Repo.DB().Where("name = ?", s.taskName).First(&primary).Error
	if primaryErr != nil && !errors.Is(primaryErr, gorm.ErrRecordNotFound) {
		return primaryErr
	}

	var legacy models.SchedulerTask
	legacyErr := repository.Repo.DB().Where("name = ?", legacyTaskName).First(&legacy).Error
	...
	if hasLegacy {
		...
		if err := repository.Repo.DB().Delete(&legacy).Error; err != nil {
			return err
		}
	}
	...
	if hasPrimary && primary.ID != 0 {
		return repository.Repo.DB().Model(&models.SchedulerTask{}).Where("id = ?", primary.ID).Updates(map[string]interface{}{
			"name":                primary.Name,
	...
	return repository.Repo.DB().Create(&primary).Error
}
...
func (s *ContentCompletionScheduler) GetStatus() adminhandler.SchedulerStatusResponse {
	var task models.SchedulerTask
	repository.Repo.DB().Where("name = ?", s.taskName).First(&task)

	s.mu.RLock()
	defer s.mu.RUnlock()

	status := adminhandler.SchedulerStatusResponse{
