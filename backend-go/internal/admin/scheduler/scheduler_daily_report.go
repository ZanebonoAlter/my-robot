...
func (s *DailyReportScheduler) UpdateInterval(interval int) error {
	...
	if err := repository.Repo.DB().Where("name = ?", "daily_report").First(&task).Error; err == nil {
...
func (s *DailyReportScheduler) GetStatus() adminhandler.SchedulerStatusResponse {
	...
	var task models.SchedulerTask
	err := repository.Repo.DB().Where("name = ?", "daily_report").First(&task).Error
