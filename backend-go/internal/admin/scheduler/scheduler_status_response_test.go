...
func TestSchedulerStatusResponseDefinition(t *testing.T) {
	...
	typ := reflect.TypeOf(adminhandler.SchedulerStatusResponse{})
...
func TestSchedulerStatusFormat(t *testing.T) {
	...
	assertSchedulerStatus(t, autoRefreshStatus, adminhandler.SchedulerStatusResponse{
		Name:          "Auto Refresh",
		Status:        "running",
		CheckInterval: 60,
	...
	assertSchedulerStatus(t, preferenceStatus, adminhandler.SchedulerStatusResponse{
	...
	completion := &ContentCompletionScheduler{
		completionService: reader.NewContentCompletionService(),
		checkInterval:     time.Hour,
		taskName:          "ai_summary",
		isRunning:         true,
	}
	completionStatus := completion.GetStatus()
	assertSchedulerStatus(t, completionStatus, adminhandler.SchedulerStatusResponse{
	...
	assertSchedulerStatus(t, firecrawlStatus, adminhandler.SchedulerStatusResponse{
...
func assertSchedulerStatus(t *testing.T, got, want adminhandler.SchedulerStatusResponse) {
	t.Helper()
