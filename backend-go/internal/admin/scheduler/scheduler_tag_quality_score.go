...
func (s *TagQualityScoreScheduler) UpdateInterval(interval int) error {
	...
	if err := repository.Repo.DB().Where("name = ?", "tag_quality_score").First(&task).Error; err == nil {
...
func (s *TagQualityScoreScheduler) runComputeCycle(triggerSource string) {
	...
	if err := repository.Repo.DB().Model(&models.TopicTag{}).Where("status = ?", "active").Count(&updatedCount).Error; err != nil {
		updatedCount = 0
	}
...
func (s *TagQualityScoreScheduler) GetStatus() adminhandler.SchedulerStatusResponse {
	...
	var task models.SchedulerTask
	err := repository.Repo.DB().Where("name = ?", "tag_quality_score").First(&task).Error
