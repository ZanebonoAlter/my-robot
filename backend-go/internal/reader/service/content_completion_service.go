...
func (s *ContentCompletionService) CompleteArticleWithMetadata(ctx context.Context, articleID uint, force bool, metadata map[string]any) (err error) {
	...
	if err := repository.Repo.DB().Save(&article).Error; err != nil {
...
func (s *ContentCompletionService) GetOverview() (*ContentCompletionOverview, error) {
	...
	for _, item := range countQuery {
		var count int64
		if err := item.query(repository.Repo.DB()).Count(&count).Error; err != nil {
			return nil, err
		}
		item.assign(count)
	}
	...
	if err := repository.Repo.DB().Where("summary_status = ?", "pending").Where("summary_processing_started_at IS NULL OR summary_processing_started_at <= ?", staleBefore).Order("summary_processing_started_at ASC").Order("created_at ASC").First(&staleArticle).Error; err == nil {
...
func (s *ContentCompletionService) persistCompletionFailure(article *models.Article, feed *models.Feed, message string) error {
	...
	return repository.Repo.DB().Save(article).Error
}
...
func (s *ContentCompletionService) claimArticleForCompletion(articleID uint, force bool, now time.Time) (bool, error) {
	...
	query := repository.Repo.DB().Model(&models.Article{}).Where("id = ?", articleID)
	if force {
