...
var queueService *service.EmbeddingQueueService
var queueServiceOnce sync.Once

func getQueueService() *service.EmbeddingQueueService {
	queueServiceOnce.Do(func() {
		queueService = service.NewEmbeddingQueueService(nil)
	})
	return queueService
}
...
func BackfillPersonMetadataHandler(c *gin.Context) {
	processed, err := service.BackfillPersonMetadata()
	if err != nil {
		c.JSON(500, gin.H{"success": false, "error": err.Error()})
		return
	}
