...
func SearchTagsHandler(c *gin.Context) {
	...
	query := repository.Repo.DB().Where("(status = 'active' OR status = '' OR status IS NULL)").
		Where("label ILIKE ?", "%"+q+"%").
...
func MergeTagsHandler(c *gin.Context) {
	...
	if err := repository.Repo.DB().First(&source, body.SourceTagID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"success": false, "error": "source tag not found"})
		return
	}
	if err := repository.Repo.DB().First(&target, body.TargetTagID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"success": false, "error": "target tag not found"})
		return
	}

	if err := service.MergeTags(body.SourceTagID, body.TargetTagID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
	}
