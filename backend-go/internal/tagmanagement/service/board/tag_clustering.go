...
func findConnectedComponents(tagIDs []uint, edges []core.SimilarityEdge) [][]uint {
...
func FindSimilarTagsByKeywordOverlap(ctx context.Context, tagIDs []uint, kwMinOverlap int, semThreshold float64) ([]core.SimilarityEdge, []core.SimilarityEdge, error) {
	if len(tagIDs) < 2 {
		return nil, nil, nil
	}
	...
	`
	if err := repository.Repo.DB().Raw(kwQuery, tagIDs, tagIDs).Scan(&kwRows).Error; err != nil {
		return nil, nil, fmt.Errorf("keyword overlap query: %w", err)
	}

	kwEdges := make([]core.SimilarityEdge, 0, len(kwRows))
	var candidatePairs []struct{ a, b uint }
	for _, r := range kwRows {
		kwEdges = append(kwEdges, core.SimilarityEdge{
			TagAID:     r.TagAID,
			TagBID:     r.TagBID,
	...
	if err := repository.Repo.DB().Raw(semQuery, args...).Scan(&semRows).Error; err != nil {
		return nil, nil, fmt.Errorf("semantic filter query: %w", err)
	}

	semEdges := make([]core.SimilarityEdge, 0, len(semRows))
	for _, r := range semRows {
		semEdges = append(semEdges, core.SimilarityEdge{
			TagAID:     r.TagAID,
			TagBID:     r.TagBID,
...
func collectUnclassifiedTagIDs(category string, limit int) ([]uint, error) {
	var relatedIDs []uint
	repository.Repo.DB().Model(&models.TopicTagRelation{}).
	...
	query := repository.Repo.DB().Model(&models.TopicTag{}).
		Where("status = 'active'").
...
func loadTagModelsMap(ids []uint) (map[uint]*models.TopicTag, error) {
	...
	if err := repository.Repo.DB().Where("id IN ?", ids).Find(&tags).Error; err != nil {
		return nil, err
	}
...
func ClusterUnclassifiedTags(ctx context.Context, category string) (*ClusteringResult, error) {
	cfg := core.NewEmbeddingConfigService().LoadClusterConfig()
	return ClusterUnclassifiedTagsWithConfig(ctx, category, cfg)
}

func ClusterUnclassifiedTagsWithConfig(ctx context.Context, category string, cfg core.ClusterConfig) (*ClusteringResult, error) {
	...
	es := core.NewEmbeddingService()

	var edges []core.SimilarityEdge
	var kwEdges []core.SimilarityEdge
	...
	for _, comp := range components {
		...
		var candidates []core.TagCandidate
		for _, id := range compIDs {
			tag := tagsMap[id]
			if tag == nil {
				continue
			}
			candidates = append(candidates, core.TagCandidate{
				Tag:        tag,
				Similarity: 1.0,
