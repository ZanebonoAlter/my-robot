...
func BackfillPersonMetadata() (int, error) {
	...
	if err := repository.Repo.DB().Where("category = ? AND status = ? AND (metadata IS NULL OR metadata = '{}'::jsonb OR metadata = '')", "person", "active").
		Limit(100).
...
Respond with JSON: {"country": "...", "organization": "...", "role": "...", "domains": [...]}`, tag.Label, tag.Description)
	...
	if err := repository.Repo.DB().Model(&models.TopicTag{}).Where("id = ?", tag.ID).
