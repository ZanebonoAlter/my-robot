...
func runFullScan(ctx context.Context) {
	...
	if err := repository.Repo.DB().Where("status = 'active' OR status = '' OR status IS NULL").Find(&tags).Error; err != nil {
	...
	for i, tag := range tags {
		...
		candidates, err := es.FindSimilarTags(ctx, &tag, tag.Category, 10, core.EmbeddingTypeSemantic)
		if err != nil {
		...
		for _, c := range candidates {
			if c.Similarity < core.MatchThreshold {
				continue
			}
			if c.Tag.ID == tag.ID {
				continue
...
func runEvaluation(ctx context.Context) {
	...
	for _, sug := range suggestions {
		if _, ok := groups[sug.ExistingTagID]; !ok {
			...
			if err := repository.Repo.DB().Select("id, label, slug, category, feed_count").First(&tag, sug.ExistingTagID).Error; err != nil {
				continue
			}
