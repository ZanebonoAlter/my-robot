...
func GetTagQueue() *TagQueue {
	...
	if instance.queue == nil {
		instance.queue = repository.NewTagJobQueue(repository.Repo.DB())
	}
	return instance
}
...
func (q *TagQueue) Enqueue(articleID uint, feedName, categoryName string) error {
	...
	return q.queue.Enqueue(repository.TagJobRequest{
		ArticleID:    articleID,
		FeedName:     feedName,
...
func (q *TagQueue) tryStart() error {
	...
	sqlDB, err := repository.Repo.DB().DB()
	if err != nil {
...
func (q *TagQueue) processJob(ctx context.Context, job models.TagJob) {
	...
	if err := repository.Repo.DB().First(&article, job.ArticleID).Error; err != nil {
