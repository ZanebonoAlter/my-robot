...
func GetTopicGraph(c *gin.Context) {
	...
	graph, err := repository.Repo.BuildTopicGraph(kind, anchor, categoryID, feedID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": err.Error()})
		return
	}
...
func GetTopicDetail(c *gin.Context) {
	...
	detail, err := repository.Repo.BuildTopicDetail(kind, slug, anchor, parseOptionalUintParam(c, "category_id"), parseOptionalUintParam(c, "feed_id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": err.Error()})
		return
	}
...
func GetTopicsByCategory(c *gin.Context) {
	...
	result, err := repository.Repo.BuildTopicsByCategory(kind, anchor, parseOptionalUintParam(c, "category_id"), parseOptionalUintParam(c, "feed_id"))
	if err != nil {
...
func GetTopicArticles(c *gin.Context) {
	...
	articles, total, err := repository.Repo.FetchTopicArticles(slug, kind, anchor, page, pageSize)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
	}
...
func GetDigestsByArticleTagHandler(c *gin.Context) {
	...
	digests, err := repository.Repo.GetDigestsByArticleTag(tagSlug, kind, anchor, limit, tagKind)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
	}
...
func GetPendingArticlesByTagHandler(c *gin.Context) {
	...
	result, err := repository.Repo.GetPendingArticlesByTag(tagSlug, kind, anchor)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
		return
	}
