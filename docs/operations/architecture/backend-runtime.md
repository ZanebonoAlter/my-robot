<<<<<<< Updated upstream:docs/reference/architecture/runtime.md
# 后端运行时与接口
...
6. `backend-go/internal/jobs/narrative_summary.go`
7. `backend-go/internal/jobs/tag_hierarchy_cleanup.go`
再回到 `docs/operations/architecture/backend-go.md` 看业务分层，会比较容易把“启动装配”和“业务链路”对上。
>>>>>>> Stashed changes:docs/operations/architecture/backend-runtime.md
