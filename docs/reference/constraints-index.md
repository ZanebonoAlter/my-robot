...
**change 涉及哪些域，在 proposal.md 头部用 `<!-- constraint-domains: 域名, ... -->` 显式声明**（域名=上表 flow 文档 basename）；档位激活后 constraint-injection 按声明注入对应约束节，代码编辑路径由 flow/standard 文档头部 `doc-impact-applies` 标签 JIT 兜底（各域辖区见标签声明）。
