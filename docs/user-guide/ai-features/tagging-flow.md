<<<<<<< Updated upstream:docs/v1.2-tag-intelligence/user-guide/tagging-flow.md
# 打标签流程全景说明
...
> **版本**：基于 `backend-go/internal/domain/topicanalysis/` 与 `topicextraction/tagger.go` 实际代码整理（2026-04 更新：新增 Phase 3.5 树间桥接、清理调度器扩展为 10 阶段、标签提取限额调整、keyword 自动复用门槛降低）  
> **阅读建议**：先看主流程图，再按需深入各子章节
...
| `BackfillMissingDescriptions` | `topicextraction/description_backfill.go` | - | int | **批量补全缺失description** |
| `formatTagPromptContext` | `topicanalysis/tag_prompt_context.go` | TopicTag | string | **统一生成LLM标签上下文，人物包含结构化属性** |
| `ExecuteTreeBridge` | `topicanalysis/tree_bridge.go` | category, budget | TreeBridgeResult | **树间桥接——扫描同类树根对执行 merge 或 parent 链接** |
...
flowchart TD
    ...
    P3A --> P3B[解决多父冲突]
    P3B --> P3C[清理空抽象节点]
    P3C --> P3_5[Phase 3.5: Tree Bridge 树间桥接]
    P3_5 --> |embedding相似度预筛<br/>全局去重 批量LLM判断<br/>merge或parent链接跨树根| P4[Phase 4: 收养更窄标签]
    P4 --> |批量处理pending队列| P5[Phase 5: 抽象标签刷新]
    P5 --> |批量处理pending队列<br/>LLM重生成label+desc| P6[Phase 6: 整树审查]
    P6 --> P6A{含14天内新关系?}
    P6A -->|否| P7[Phase 7: Description回填]
...
| Phase 2 Flat Merge | `tag_cleanup.go` `ExecuteFlatMerge` | 是 | 同类 abstract 标签去重 |
| Phase 3 层级修剪 | `tag_cleanup.go` 三个函数 | 否 | 孤儿关系 / 多父 / 空抽象 |
| Phase 3.5 树间桥接 | `tree_bridge.go` `ExecuteTreeBridge` | 是 | embedding 预筛 + LLM 批量判断树根对 merge/parent，跨树合并或链接 |
| Phase 4 收养更窄标签 | `queue_batch_processor.go` `ProcessPendingAdoptNarrowerTasks` | 是 | 处理 pending 的 adopt_narrower 队列任务 |
| Phase 5 抽象标签刷新 | `queue_batch_processor.go` `ProcessPendingAbstractTagUpdateTasks` | 是 | 处理 pending 的 abstract_tag_update 队列任务 |
| Phase 6 整树审查 | `hierarchy_cleanup.go` `ReviewHierarchyTrees` | 是 | 含 windowDays 内新关系的 minDepth=2 标签树 |
| Phase 7 Description回填 | `description_backfill.go` `BackfillMissingDescriptions` | 是 | active 标签中 description 为空的，从关联文章获取上下文生成 |

核心文件: `jobs/tag_hierarchy_cleanup.go`（调度器）、`topicanalysis/tag_cleanup.go`（Phase 1-1.7-3）、`topicanalysis/tree_bridge.go`（Phase 3.5）、`topicanalysis/queue_batch_processor.go`（Phase 4-5）、`topicanalysis/hierarchy_cleanup.go`（Phase 6）、`topicextraction/description_backfill.go`（Phase 7）

Phase 3 清完脏数据后再桥接，减少干扰。

### Phase 3.5: Tree Bridge 树间桥接

Phase 3.5 在层级修剪（Phase 3）完成、收养更窄标签（Phase 4）开始前执行，扫描同类标签森林中应连接但尚未连接的树根对。流程如下：

1. **收集树根**：通过 `BuildTagForest(category, minDepth=1)` 收集所有有子节点的树根，同时查询无任何关系的孤立抽象标签（散落节点）。
2. **嵌入预筛**：对每个树根调用 `FindSimilarAbstractTags`（limit=15），仅保留相似度 ≥ 0.78 的候选标签对。
3. **全局去重**：使用 `min(idA,idB)|max(idA,idB)` 字符串 key 确保每对树根只在全局出现一次，按相似度降序排列，上限 50 对。
4. **批量 LLM 判断**：将去重后的树根对分批次（≤20 对/批）送 LLM，每对包含根标签上下文、前 8 个子节点和相似度。LLM 返回 merge（合并）/ parent_A（B 挂在 A 下）/ parent_B（A 挂在 B 下）/ skip 四种动作。
5. **安全执行**：先执行所有 merge（调用 `MergeTags`，维护 skipSet 记录已消失标签），再执行 parent 链接（调用 `linkAbstractParentChild`，跳过 skipSet 中的标签）。

**预算管理**：Phase 3.5 配额为 5 次 LLM 调用（`budget.SetPhaseQuota("phase3_5", 5)`），每批 LLM 调用前检查预算和超时。

**执行顺序**：按 event → keyword → person 类别处理，任意类别预算耗尽或超时则停止后续类别，不影响 Phase 4-7 继续执行。
...
新标签 → Embedding三级匹配 → 有候选则LLM判断(merges/abstracts/none三个必填数组)
               ...
               异步: 生成embedding → 层级匹配 → EnqueueAdoptNarrower入队 → 入队刷新
                                     ↓
                 定时清理(10阶段): zombie → 零文章清理 → 低质量keyword清理 → 过期零分清理 → flat merge → 层级修剪 → 树间桥接(Phase 3.5) → 收养窄标签 → 抽象刷新 → 整树审查 → description回填
                                     ↓
               收养队列: 5s轮询 + Phase4批量处理 → batchJudgeNarrowerConcepts批量LLM → 收养更窄标签
                                     ↓
               层级匹配: 跨层去重 + 深度限制 → AI判断父子
               ...
               链接成功: 解决多父冲突 → embedding动态管理 → 父标签入队刷新
```
>>>>>>> Stashed changes:docs/user-guide/ai-features/tagging-flow.md
