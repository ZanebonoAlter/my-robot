...
### Requirement: (removed) — 旧增量匹配逻辑
**Reason**: 增量贪心匹配（`MatchAndSaveRelations` + `shouldWriteRelation` + `competitiveFilter`）已被相邻天二分图最优匹配取代。旧逻辑的累积误差和 merge 泛滥问题在新算法中从根本上解决。
**Migration**: 旧的 `MatchAndSaveRelations` 函数替换为 `RebuildBoardRelations`。旧的 `shouldWriteRelation`、`competitiveFilter`、`hasContinuationInIntermediateDays`、`matchCandidate` 删除。`BackfillRelations` 重写为基于二分图匹配的版本。运行 `BackfillAllRelations()` 可将所有 board 的 relation 迁移到新拓扑。
