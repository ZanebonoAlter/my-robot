...
独立设置工作区已落地，桌面/窄屏布局、路由恢复和订阅源主从编辑均可用，但浏览器回归发现部分“已拆分”仅完成了导航层，内容层仍复用旧聚合面板。

- **完成 AI 设置内容拆分**：`AI 模型` 只展示提供商管理，不得重复渲染能力路由和板块匹配阈值
- **落实长列表治理**：队列和阅读偏好必须实际限制首屏记录量，不能只在导航层拆分后继续渲染两千像素以上长列表
- **完善定时任务文案**：补齐任务中文名称和 `idle` / `running` 等状态本地化，技术标识仅作为次要信息
- **继续验证 TopicGraph Canvas**：外围界面已响应主题，但节点、标签和边仍需完成像素级双主题验收
