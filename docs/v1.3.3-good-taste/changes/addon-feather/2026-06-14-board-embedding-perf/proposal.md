...
版块 embedding 匹配存在严重性能问题和功能缺失：(1) `MatchTopicTag` 每次调用都全量加载所有版块辅助标签和 embedding 到 Go 内存做 O(N×T×B×d) 暴力余弦比较，无缓存、无索引；(2) `semantic_labels.embedding` 的 HNSW 索引被删除后未重建（维度 2560 超过 HNSW 2000 维限制）；(3) 版块升级只支持"发现新版块"模式，缺少"扩充已有版块"方向；(4) backfill 逐条顺序处理无法并行；(5) `sqlMergeMatcher` 每次调用都从 DB 加载全部活跃辅助标签的 `merge_embedding` 向量（~216 MB），是打标签流程的主要瓶颈（单次 5.2s）。
...
- 新增内存缓存层：
  ...
  - **merge embedding 向量缓存**（`AuxiliaryLabelService` 内）——TTL 10 分钟，新增/禁用标签时失效；消除 `sqlMergeMatcher` 的 216 MB 重复传输（5.2s → ~0ms）
...
- `aux-label-merge-cache`: 辅助标签 merge embedding 向量缓存，消除 `sqlMergeMatcher` 的重复 DB 查询和 parsePgVector 开销

### Modified Capabilities

- `tag-to-board-matching`: 匹配流程增加缓存和元数据预过滤，backfill 改为批处理并行
- `board-upgrade`: 拆分为两个方向（发现新版块 / 扩充已有版块），前端增加模式选择
- `tag-embedding-management`: 优化 `parsePgVector` 调用，减少重复解析；merge embedding 缓存存储已解析数据
...
- `backend-go/internal/domain/tagging/auxiliary_label_service.go`：辅助标签元数据缓存 + merge embedding 向量缓存 + 统一失效逻辑
