...
| **软降级** | 灰显 + 左对齐离群标记（行可见） | ✅ **选** |
| 软降级 + 探究行 | 上者 + hover 贴合度数值 | ✅ **选** |
| 剔除 | 不渲染 | ❌ 删信息，单用户产品损失叙事完整性 |
| 重组 section | tag 回流拆分 | ❌ 治源头，scope 跳档（Non-Goal）|

软降级保证：跑题 thread 不刺眼（灰显），但始终可见可读（信息不丢，用户可甄别）。跑题 thread 的 section 底部显示「可能跑题的线索 N 条」提示行。

**展示分层**：跑题 thread 标题照常显示但**降级样式**（灰 token），并在标题**左侧**对齐一个「可能跑题」alert 图标（状态标记，带 aria-label）；贴合度数值 + 中文标签（"贴合"/"可能跑题"）只在 hover/展开时呈现——与 anchor 徽章一致。

**【设计 A 修订 · 2026-06-27】** 初版 D4 写的是「默认折叠 + hint 行可点击批量展开离群 thread」。评审发现两个问题：(1) alert 图标原放在 thread 右侧 meta，与计数/箭头挤在一起「根本看不见」；(2) hint 行点击复用 toggleThread，只展开关联文章——而「展开文章对提示跑题无意义」，用户要看文章照常点单条 thread。修订为设计 A：跑题 thread 行保持可见（灰显，不折叠），alert 图标移至标题左侧；hint 行从可点 `<button>` 降级为 `<p>` 纯状态说明（「可能跑题的线索 N 条」，不可点）。约定沉淀见 `standard/frontend/interaction-conventions.md` §1§2。
...
- **[默认折叠可能让用户错过信息]** → 设计 A 已取消默认折叠：跑题 thread 行保持可见（灰显 + 左侧 alert 图标），底部「可能跑题的线索 N 条」纯状态说明，不折叠不隐藏。信息始终可读，只是视觉降级。
