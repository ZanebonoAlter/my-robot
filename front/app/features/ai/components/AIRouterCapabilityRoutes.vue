      ...
      <div v-for="capability in ctx.capabilityOrder" :key="capability" class="px-5 py-4">
        ...
        <div v-else class="space-y-1.5 mb-3">
          <div v-for="(providerId, index) in ctx.routeSummary(capability)" :key="providerId"
            ...
            :class="[
              providerId === ctx.primaryProviderId ? 'border-[var(--color-border-subtle)]/80 bg-[var(--color-bg-hover)]/50' : 'border-gray-200 bg-gray-50/50 hover:bg-gray-100/60',
          ...
          >
            <span class="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0"
              :class="index === 0 ? 'bg-[var(--color-bg-sunken)] text-white' : 'bg-gray-300 text-gray-600'">{{ index + 1 }}</span>
            <Icon icon="mdi:drag" width="12" height="12" class="text-gray-300 shrink-0" />
            <div class="flex-1 min-w-0">
              <span class="text-sm truncate" :class="providerId === ctx.primaryProviderId ? 'font-medium text-[var(--color-text-primary)]' : 'text-gray-700'">{{ ctx.providerName(providerId) }}</span>
            </div>
            <span v-if="providerId === ctx.primaryProviderId" class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[var(--color-bg-elevated)] text-[var(--color-text-secondary)] shrink-0">主</span>
        ...
        <div class="flex flex-wrap gap-1.5">
          <button v-if="ctx.primaryProviderId && !ctx.routeSummary(capability).includes(ctx.primaryProviderId)"
            class="px-2 py-0.5 text-[11px] font-medium rounded border border-[var(--color-border-subtle)] bg-[var(--color-bg-hover)] text-[var(--color-text-primary)] hover:bg-[var(--color-bg-elevated)] transition-colors"
