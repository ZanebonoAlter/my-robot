...
max_sim 规则的方向性校验：计算 tag identity embedding 与 board embedding 的 cosine 相似度，低于阈值时标记 direction_mismatch=true。仅对 max_sim 规则生效。

## ADDED Requirements

### Requirement: Direction check for max_sim matches

evaluateSemanticBoardMatches 在 max_sim 规则匹配成功后，计算 tag identity embedding 与 board embedding 的 cosine 相似度。当 cosine < DirectionSimThreshold（默认 0.5）时，标记 direction_mismatch=true。仅对 max_sim 规则生效，hit_rate、weighted、direct_hit 不受影响。

#### Scenario: max_sim match passes direction check
- **WHEN** tag matches board via max_sim rule AND cosine(tag_identity_embedding, board_embedding) >= 0.5
- **THEN** direction_mismatch=false, match recorded normally

#### Scenario: max_sim match fails direction check
- **WHEN** tag matches board via max_sim rule AND cosine(tag_identity_embedding, board_embedding) < 0.5
...
#### Scenario: non-max_sim match
- **WHEN** tag matches board via hit_rate, weighted, or direct_hit rule
