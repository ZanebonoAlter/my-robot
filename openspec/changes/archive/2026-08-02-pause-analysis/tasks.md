...
- [x] 10.1 apply 启动时跑 `bash scripts/doc-impact.sh suggest` 预勾选受影响文档，`bash scripts/doc-impact.sh context` 注入 flow 业务约束（过程性动作已补；最终声明 flow api 以 verify exit 0 为准）
...
- [x] 10.3 归档前跑 `bash scripts/doc-impact.sh verify` 对账（exit 0：声明 flow api / 文件 1 个）
...
- [x] 11.8 手动验收：启动后端+前端 → 点顶部栏暂停，确认 content_completion/firecrawl 不再 lease、tag worker 不消费、auto_refresh 仍入库、favicon 出现 ⏸、重启后仍暂停；再点恢复 → 堆积任务续跑、favicon 复原（用户人工确认）
- [x] 11.9 `bash scripts/doc-impact.sh verify` → 期望：文档节声明与实际改动一致（归档前重跑 exit 0：声明 flow api / 文件 1 个；check-standards F 段「doc-impact 通过 pause-analysis」）
