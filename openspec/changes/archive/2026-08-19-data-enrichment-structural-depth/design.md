...
10. **博查 key 界面可配置（追加需求）**：博查 key 从「config.yaml + env」升级为「**DB/UI 优先 + env/config.yaml 兜底**」，照 Firecrawl 已验证的 `ai_settings` 模式（用户要求界面可配）。①**存储**：`ai_settings` 表 `bocha_config` key（jsonb `{api_key, endpoint, enabled}`），照 `loadConfigByKey`/`saveConfigByKey` 加 `LoadBochaConfig`/`SaveBochaConfig`（`config_store.go`）。②**后端 API**：`GET/POST /api/settings/bocha`（`GetBochaSettings`/`SaveBochaSettings`，照 `/settings/rsshub`、`/settings/proxy` 模式；`GET /settings` 全量返回白名单 `ai_handler.go:308` 加 `bocha_config` 让前端拉到）。③**动态读（界面改了不重启）**：`BochaWebSearcher` 改持 `BochaConfigProvider`（`func()(apiKey, endpoint string)`），每次 `Search` 现读——优先级 **DB(`bocha_config`) > env(`BOCHA_API_KEY`/`BOCHA_ENDPOINT`) > config.yaml(`bocha.*`) > 空**；空 key 返回 `not configured` error，`executeWebSearch` 降级错误 JSON（同 Noop 语义）。理由：Firecrawl 即每次 job 读 DB、界面改即时生效；博查对齐，避免「改 key 要重启」。`NoopWebSearcher` 保留仅作测试桩。④**前端**：设置页加 `bocha` section——`SettingsSectionBocha.vue`（照 `SettingsSectionFirecrawl` + `FirecrawlConfigPanel` 模式，表单 `api_key`+`endpoint`+`enabled`，调 `/api/settings/bocha`）；`settings.vue` 的 `sectionComponents` 加 `'bocha'`；`SettingsWorkspace.vue` 左侧导航登记。**风险**：博查 key 入 `ai_settings` 表明文存——与 Firecrawl key 既有约定一致，可接受；未来要加密存储再统一改（非本 change 范围）。
