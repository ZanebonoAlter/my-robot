# constraint-injection delta（constraint-injection-tier-b）

> ⚠️ 归档依赖：本 delta 的「每 turn system prompt 强制注入」MODIFIED 块已 rebase 至 constraint-domain-declaration（cdd）的目标状态（cdd 已于 2026-08-23 归档，主 spec 为新基线）。
...
档位状态 SHALL 持久化：每次档位激活（命令命中或 skill 路径命中）时 MUST 向事实库记 `mode.set` 事件（payload 含 mode 与绑定 change）。会话边界的恢复按 reason 区分：`resume`（/resume 切换目标会话）与 `reload`（同会话扩展重载）SHALL 清零后依据事实库恢复（两段式：同 sessionId 最近一条 → 全局最近一条）；`startup` SHALL NOT 清零（pi-subagents 派发共用模块实例），但档位为空时 SHALL 仅按同 sessionId 的最近一条 `mode.set` 恢复（pi 进程重启恢复会话的真实路径），MUST NOT 全局兑底（全新会话不得继承其他会话的档位）；`new`/`fork` SHALL 维持清零语义（不恢复）。无法恢复（无记录、记录过期或绑定 change 目录已不存在）MUST 回落未激活档。恢复的档位与绑定遵循既有回落规则（change 归档后自动回落、写 change 目录修正绑定）。
...
- **THEN** 清零后按两段式恢复档位与 change 绑定，无需重新触发阶段命令

#### Scenario: resume 无法恢复回落未激活

- **WHEN** resume 时事实库无可用 mode.set（无记录/过期），或记录绑定的 change 目录已不存在
