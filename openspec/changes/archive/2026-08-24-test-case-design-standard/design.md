...
跨 change 断链分析：Scenario 节拍归档后进主 specs（openspec 流程强制读）、测试代码天然存在（change-scope 跑影响包时连锅端）、但 **test-cases.md 故事文档与 Scenario→测试文件映射躺在 archive 里无人知晓**——改契约（MODIFIED/REMOVED）时旧测试可能仍断言旧契约，跑绿 ≠ 对。三个决定（用户拍板）及其选型：

- **继承表放 test-cases.md**（非 tasks 验证节）：test-cases.md 是故事账本，继承与调整是故事的一部分；tasks 验证节只放机器对账的 `| Scenario | 测试文件 |` 表（scenario-trace 格式约定不动）。
- **test-assets.sh 反向索引而非改主 specs 格式**：把「测试落点」写进主 specs 会改既有 specs 格式（漂移风险、存量 100+ spec 全要回填）；脚本从 archive 现存文件机械重建（grep/awk 只读只判，学 scenario-trace.sh / change-scope.sh 的「机械无猜测」），零存量迁移成本。
- **问句⓪ 定位为条件问句**（非第五问句）：仅改契约时激活，平时保持四问句主体——摘要体积与既有断言锚点（冒烟锚「四问句/双轨」结构词）均不受影响。

机制决策（D1-D4）不受影响；检查⑤不动（继承表合规性靠注入提醒 + 人审，不进门禁——避免归档门禁膨胀）。
