...
> ✅ 已修复（2026-08-23，用户授权跳过 change 直接改）：scripts/doc-impact.sh 规则 2 兜底处加 `git check-ignore -q` 判断——ignored 文件存在于工作树即视为已更新。验证三分支：ignored+存在→过（bug 修复）；有提交历史→过（事后补归档兜底不变）；路径不存在→规则 5 FAIL 不变。附带发现：修复前该 FAIL 分支几乎只对 gitignored 文件可达（未跟踪非 ignored 文件必在 changed 集合）。check-standards F 段全量回归 116/0。
