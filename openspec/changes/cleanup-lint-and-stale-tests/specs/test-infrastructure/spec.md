...
仓库后端须维持全量门禁零失败：`golangci-lint run ./...` 与 `go test ./...`（含 testcontainer 集成测试）在全干工作树上执行须零失败。既存 lint 债（gofmt / unused / errcheck 等）与 pre-existing 坏测试须即时清理，不得积压阻塞《开发执行规范》§11.1 归档门禁。新增代码引入新 lint / 测试失败时，须在同一 change 内修复。
