...
- 配置：博查 key 照 airouter 模式（`configs/config.yaml` + 环境变量 `BOCHA_API_KEY`），无 key 时优雅降级回 Noop
...
- **配置**：`configs/config.yaml` 增博查配置段；新增环境变量 `BOCHA_API_KEY`
- **依赖**：不引入新三方依赖（博查走标准 HTTP，fetch_page 复用 reader 既有 readability）
- **部署影响**：合并后用户可见行为变化——增强分析从"A 股产业点评"变为"结构化深度剖析"；需用户配博查 key（无 key 则 web_search 降级、深度层仍产但证据链弱）；旧 ETF source 配置失效需清理
