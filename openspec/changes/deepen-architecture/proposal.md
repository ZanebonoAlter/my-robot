...
- **统一 SSE 事件流**：用一个 `useEventStream()` 组合函数管连接，其他模块订阅类型化事件。替代目前四个独立连接各自过滤的模式。**将 WebSocket 替换为已有的 SSE 方案。**
...
- **组件健康度**：`TopicGraphPage.vue` (2286 行) 拆解；`FeedLayoutShell.vue` 消除 7 处不必要的 `any`。
...
- `TopicGraphPage.vue` (2286行)、`TagsPage.vue` (1879行) 远超合理阈值，需要拆解为 composable + 子组件。
...
- **前端破坏性变更**: WebSocket 连接代码替换为 SSE；Store mutation API 变更（调用者需 await API）。
