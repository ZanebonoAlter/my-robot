...
span 名 SHALL 与 AICallLog.operation 对齐（`workflow.` 前缀 + operation 值）：`workflow.daily_report.cluster_tags` / `.highlights` / `.threads` / `.merge_arbitration` / `.section_embedding`。

> 具体 span 插入点（函数级）由 apply 阶段对着 `internal/topicgraph/service/daily_report_*.go` 实际代码决策，本 requirement 锁定命名与拓扑目标。
...
### Requirement: Asynchronous watch evaluation linked to report trace

`topic_watch` 的异步评估 SHALL 通过 `tracing.GoWithTrace` 接入 trace，用 `parent_trace_id` attribute 记录来源日报 trace，不再完全脱离 trace 上下文。

> 跨 goroutine 不挂子 span（OTel 不推荐），用 `parent_trace_id` 软关联。

#### Scenario: watch 评估 span 带 parent 关联

- **WHEN** 日报事务后异步触发 topic_watch 评估
- **THEN** 生成的 watch span SHALL 携带 `parent_trace_id` attribute 指向日报 trace，可在聚合端点按 session 关联呈现
