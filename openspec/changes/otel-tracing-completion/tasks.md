...
- [ ] 1.1 新增 `GET /api/ai/sessions/:session_id` handler（`internal/admin/handler/`）
- [ ] 1.2 聚合查询：`ai_call_logs WHERE session_id=?` → trace_id 集合（去重）→ `otel_spans WHERE trace_id IN (...)` → 复用 `tracing.BuildSpanTree`
- [ ] 1.3 响应 schema：`summary`（call_count/span_count/started_at/ended_at/total_tokens/error_count）+ `call_logs` + `timeline`
...
- [ ] 2.1 `GenerateDailyReport`（orchestrator.go:18）编排入口开 root span `workflow.daily_report.generate`
- [ ] 2.2 本地节点补薄 span（记耗时/拓扑）：
...
- [ ] 2.3 LLM/embedding 节点补 span（对齐 `AICallLog.operation` + `workflow.` 前缀）：
...
- [ ] 2.4 验证 trace 内 span 形成父子拓扑（非孤立）；并发节点共享正确 parent
...
<!-- doc-impact: flow api database architecture standard -->

> 域说明（doc-impact verify 启发式扫全工作树 base=HEAD，无法区分 change；当前工作树含其他 active change 脏文件）：
> - flow：本 change 改 `topicgraph/service/daily_report_*.go`（编排埋点），可观测层不改业务流，flow 文档无需更新
> - api：本 change 新增 `docs/reference/api/ai-sessions.md`（下方 checkbox）
> - database：`internal/models/ai_models.go` 改动属其他 active change，本 change 无 schema 变更
> - architecture：本 change 更新 `docs/reference/architecture/tracing.md`（下方 checkbox）
> - standard：`docs/reference/standard/backend/ai-logging.md` 改动属其他 active change，本 change 未改 standard

- [ ] `docs/reference/api/ai-sessions.md`：新增 `GET /api/ai/sessions/:session_id` 端点文档
- [ ] `docs/reference/architecture/tracing.md`：「当前问题与下一步建议」勾掉编排埋点 + §5 异步 helper 勘误（helper 实测不存在）

## 5. 验证

- `cd backend-go && go vet ./... → 零警告`
- `cd backend-go && golangci-lint run ./... → 零失败`
- `cd backend-go && go test ./internal/admin → PASS`
- `cd backend-go && go test ./internal/topicgraph → PASS`
- `cd backend-go && go build ./... → 成功`
- 触发一次日报生成 → `GET /api/ai/sessions/<session_id>` → 响应 `timeline` 含 `workflow.daily_report.*` span（编排埋点校验）
