...
- [x] 3.4 主循环：遍历 specs → 对每张表执行 `db.Raw("SELECT <投影> FROM <table> <where>").Rows()` → `rows.Columns()` + `rows.Scan(...)` 到 `[]sql.RawBytes` → 逐行套用 sanitizers → 拼 `INSERT INTO <table> (cols) VALUES (...), (...);` → 末尾 `SELECT setval(pg_get_serial_sequence('<table>','id'), COALESCE(MAX(id),0)+1, false) FROM <table>;`
...
- [x] 4.1 新建 `Dockerfile.demo`（三阶段，design D9）：
...
- [x] 4.2 新建 `demo/entrypoint.sh`（design D5）：
  - `#!/bin/sh` + `set -e`
  - 后台启后端 `/app/syntopica &`
...
- [x] 9.10 `curl -sf http://localhost:5000/health` → `{"status":"healthy","database":"connected"}`
- [x] 9.11 `curl -sf http://localhost:5000/api/categories` → 返回 JSON 数组（seed 数据）
- [x] 9.12 `curl -sf http://localhost:5000/api/semantic-boards` → 返回板块列表
- [x] 9.13 只读验证：`curl -X POST http://localhost:5000/api/categories -H "Content-Type: application/json" -d "{\"name\":\"x\"}"` → HTTP 405 + `{"error":"read-only demo"}`
- [x] 9.14 只读验证：`curl -X POST http://localhost:5000/api/daily-reports/generate` → HTTP 405
- [x] 9.15 浏览器访问 `http://localhost:5000/` → 首页有文章列表；`/topics` 有图谱；`/tags` 有板块且能进侦探墙；`/settings` 各 section 不崩
