...
- 理由：section↔topic 锚定阈值 0.30 是"够上"的及格线；thread↔section 是"是否忠于本 section 主题"，应比跨话题锚定严。0.20 < 0.30 给出 0.10 的余量，让"贴 section 但略远"的 thread（如 0.15-0.20）不被误降级。
- 标定方法：落库后查现网 thread.fit_distance 分布，找分布的"自然断点"（贴合聚集 vs 离群聚集的分界），据此微调常量。单测覆盖 `0.20 ± 0.001` 边界。
...
- **[伞形 section 的跑题 thread 可能误判为正常（假阴性）]** → B1 天然对伞形宽容（标题声明广度），这是特性非缺陷；伞形 section 内真正跑题的 thread（如案例的机器人混入 OpenAI）距离仍会显著超阈值（标题"OpenAI 发布管控"与"机器人控制器"距离远大于 0.20），假阴性风险低。Task 1 标定时用本案例做真阳验证。
