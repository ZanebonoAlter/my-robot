...
Section 级别的生命周期管理：通过 `cluster_tag_ids` Jaccard 相似度匹配跨天 section，推导 status（emerging/continuing/ending），提供 timeline 和 lifecycle API，前端展示话题总览 Gantt 图和 Section Lifecycle Panel。
